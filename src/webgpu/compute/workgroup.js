export async function main() {
  ...
  for (let i = 0; i < numResults; ++i) {
    const start = i % numThreadsPerWorkgroup === 0;
    const id = Math.floor(i / numThreadsPerWorkgroup) ;
...
---------------------------------------`, id);
    ...
    log(`${i.toString().padStart(3)}:      ${get3(workgroup, i)}      ${get3(local, i)}   ${get3(global, i)}`, id);
  }
...
function log(text, id) {
    ...
    let dom = document.querySelector('.group-'+id);
    if (!dom) {
      dom = document.createElement('div');
      dom.className='workgroup group-'+id;
      document.querySelector('.compute').appendChild(dom);
    }
